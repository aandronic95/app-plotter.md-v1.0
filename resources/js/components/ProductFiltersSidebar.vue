        ...
        <div v-if="categories && categories.length > 0">
            ...
            <div class="space-y-2">
                <template v-for="category in categories" :key="category.id">
                    ...
                    <!-- Subcategories -->
                    <div
                        v-if="category.children && Array.isArray(category.children) && category.children.length > 0"
                        class="ml-4 mt-1 space-y-1"
                    >
                        <button
                            v-for="child in category.children"
                            :key="child.id"
                            @click="handleCategoryClick(child.slug)"
                            :class="[
        ...
        <div>
            ...
            <div class="space-y-3">
                <div class="flex items-center gap-2">
                    <Label for="min_price" class="w-20 text-sm">{{ t('from') }}:</Label>
                    <Input
                        id="min_price"
                        v-model="minPrice"
                        type="number"
                ...
                <div class="flex items-center gap-2">
                    <Label for="max_price" class="w-20 text-sm">{{ t('to') }}:</Label>
                    <Input
                        id="max_price"
                        v-model="maxPrice"
                        type="number"
            ...
            <div class="space-y-3">
                <div class="flex items-center space-x-2">
                    <Checkbox
                        id="in_stock"
                        :checked="inStock"
                        @update:checked="(checked) => { inStock = checked as boolean; applyFilters(); }"
                    />
                    <Label
                        for="in_stock"
            ...
            <div class="space-y-3">
                <div class="flex items-center space-x-2">
                    <Checkbox
                        id="has_discount"
                        :checked="hasDiscount"
                        @update:checked="(checked) => { hasDiscount = checked as boolean; applyFilters(); }"
                    />
                    <Label
                        for="has_discount"
                        class="text-sm font-normal cursor-pointer"
                ...
                <div class="space-y-2 pt-2">
                    <div class="flex items-center gap-2">
                        <Label for="min_discount" class="w-20 text-xs">Min %:</Label>
                        <Input
                            id="min_discount"
                            v-model="minDiscount"
                            type="number"
                    ...
                    <div class="flex items-center gap-2">
                        <Label for="max_discount" class="w-20 text-xs">Max %:</Label>
                        <Input
                            id="max_discount"
                            v-model="maxDiscount"
                            type="number"
            ...
            <div class="space-y-3">
                <div class="flex items-center space-x-2">
                    <Checkbox
                        id="is_featured"
                        :checked="isFeatured"
                        @update:checked="(checked) => { isFeatured = checked as boolean; applyFilters(); }"
                    />
                    <Label
                        for="is_featured"
                        class="text-sm font-normal cursor-pointer"
                ...
                <div class="flex items-center space-x-2">
                    <Checkbox
                        id="is_new"
                        :checked="isNew"
                        @update:checked="(checked) => { isNew = checked as boolean; applyFilters(); }"
                    />
                    <Label
                        for="is_new"
                        class="text-sm font-normal cursor-pointer"
            ...
            <div class="space-y-3">
                <div class="flex items-center space-x-2">
                    <Checkbox
                        id="low_stock"
                        :checked="lowStock"
                        @update:checked="(checked) => { lowStock = checked as boolean; applyFilters(); }"
                    />
                    <Label
                        for="low_stock"
                        class="text-sm font-normal cursor-pointer"
